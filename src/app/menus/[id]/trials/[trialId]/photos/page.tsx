...
export default async function PhotosPage({ params }: PageProps) {
  ...
  const [menuRes, trialRes] = await Promise.all([
    ...
    client.from(T.TRIALS).select(`*, trial_photos:${T.TRIAL_PHOTOS}(*)`).eq('id', trialId).single(),
  ])
  ...
  const photos = (trial.trial_photos ?? []) as Array<{
    id: string
    trial_id: string
    photo_type: string
    storage_path: string
    ...
    <div className="flex items-center gap-3 py-3">
      ...
      <div>
        ...
        <p className="text-xs mt-0.5" style={{ color: '#9ca3af' }}>
          {safeMenu.name} — v{trial.version}（{formatDate(trial.trial_date)}）
        </p>
      </div>
    </div>
  )
